...
ENTRYPOINT exec java -cp "*" -Xms${MEMORY} -Xmx${MEMORY} edu.stanford.nlp.pipeline.StanfordCoreNLPServer -port ${PORT} -quiet -timeout ${TIMEOUT} -preload ${PRELOAD_ANNOTATORS} -annotators ${ANNOTATORS}